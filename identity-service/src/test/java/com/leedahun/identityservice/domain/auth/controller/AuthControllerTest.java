...
//        assertThat(setCookie).contains("Path=/api/auth/refresh");
    ...
    @Test
    @DisplayName("[POST /api/auth/email-verification/request] 인증 메일 전송 성공 시 201 CREATED와 본문을 반환한다")
    void send_email_verification_success() throws Exception {
        ...
        mockMvc.perform(post("/api/auth/email-verification/request")
                ...
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.status").value("201"))
                .andExpect(jsonPath("$.message").value(SuccessMessage.EMAIL_SEND_SUCCESS.getMessage()))
    ...
    @Test
    @DisplayName("[POST /api/auth/email-verification/confirm] 코드 일치(VERIFIED) 시 200 OK와 '인증 성공' 메시지 및 상태를 반환한다")
